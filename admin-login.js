...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function verifyRoleAndRedirect(userEmail) {
        ...
        .then(data => {
            ...
            if (data.role === REQUIRED_ROLE) {
                ...
                setStatus(`Signed in as ${userEmail}, but this Gmail is not authorized as admin.`, true);
                showModal('Access Denied: This verified Gmail is not the admin.');
            }
        })
        .catch(err => {
    ...
    // Automatic sign-in: if the admin Gmail is already signed in with Google
    ...
    if (isSignInWithEmailLink(auth, window.location.href)) {
        ...
        if (!email) {
            email = window.prompt('Please provide your admin Gmail for confirmation');
        }
